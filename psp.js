/* ============================================================
   THE MACHINE — Persona XMB Edition
   ============================================================ */
...
function init() {
  ...
  new GLTFLoader().load('psp.glb', (gltf) => { setup(gltf.scene); }, undefined, () => { SK.fireReady(); });
  ...
  function largestFaceNormal(mesh) { /* (Omitted for brevity - same exact logic) */
  ...
  function buildScreenUVs(mesh, nrm, right, up) { /* (Same logic) */
  ...
  // --- Input & Render Loop Below (unchanged Raycaster logic) ---
  ...
  function fitToView() {
    ...
    const fill = window.innerWidth < 720 ? 0.95 : 0.60;  // Bigger PSP
